...
<html>
	...
	<body>
		<div data-role="page" id="pageHome">
			<div data-role="navbar" id="navigationBar">
				<ul>
					<li><a id="nav_query" data-ajax="false" class="ui-btn-active">往来账目</a></li>
				...
				<form method="post">					
					...
					<div class="ui-field-contain">
						...
						<select name="allShow" id="allShow" data-native-menu="false" data-theme="b">
							...
							<option value="1">应收款为零</option>
							<option value="2">应收款不为零</option>
						</select>
					</div>
					<div class="ui-field-contain">
						<label><input id="baohanyushouyufu" name="baohanyushouyufu" type="checkbox">包含预收预付</label>
					</div>
					<br/>
	...
	<script type="text/javascript">
		...
		function sendForSupplyUnit(supplyUnitCode,pageNum,backCall){
			var organization =  $("#jigou").data("orgCode");
			var requestData = "action=action_wlzmb_unit&OperatorID=<%=OperatorID%>&organization="+organization+"&pageNum=1&itemsInEachPage=5"
			if(typeof(pageNum)!="undefined" && typeof(supplyUnitCode)!="undefined"){
				requestData = "action=action_wlzmb_unit&OperatorID=<%=OperatorID%>&organization="+organization+"&pageNum="+pageNum+"&itemsInEachPage=5&supplyUnitCode="+supplyUnitCode
			}
			$.ajax({
				url: contextPath+"/BusinessServlet",
				data: requestData,
