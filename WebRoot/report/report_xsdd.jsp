...
<html>
			...
			<div data-role="content" data-theme="b">
				<form method="post">
					...
					<div class="ui-field-contain">
						...
						<a id="kehuHref"><input readonly="readonly" type="text" name="kehu" id="kehu"></a>
					</div>
					<div class="ui-field-contain">
						<label for="fahuocangku">发货仓库：</label>
						<select name="fahuocangku" id="fahuocangku" data-native-menu="false"></select>
					</div>
					<div class="ui-field-contain">
					...
					<div class="ui-field-contain">
						...
						<select name="fahuoleixing" id="fahuoleixing" data-native-menu="false"></select>
					</div>
					<br/>
	...
	<script type="text/javascript">
		...
		var cgddcode;//机构编码 （用于判断机构编码是否改变）
		var orgTreeObj;//树组件
		...
		$(function(){
			...
			$("#fahuocangku").selectmenu("disable",true);
			$("#fahuoleixing").selectmenu("disable",true);
				...
				if(popType==1 && needRefresh){
					...
					setTimeout(function(){
						var backCall = function(){
							...
							$("#fahuocangku").selectmenu("enable",true);
							$("#fahuoleixing").selectmenu("enable",true);
			...
			$("#saveBtn").bind("click",function(){
				...
				//验证客户
				var kehu = $("#kehu").data("orgCode");
				if(typeof(kehu) == "undefined" || kehu==null || kehu.length==0){
					showTip("请选择客户",true);
					return;
				}
				
				//验证发货仓库
				var fahuocangku = $("#fahuocangku").val();
				if(typeof(fahuocangku) == "undefined" || fahuocangku==null || fahuocangku.length==0){
					showTip("请选择发货仓库",true);
					return;
				}
				...
				//验证发货类型
				var fahuoleixing = $("#fahuoleixing").val();
				if(typeof(fahuoleixing) == "undefined" || fahuoleixing==null || fahuoleixing.length==0){
					showTip("请选择发货类型",true);
					return;
				}
					...
					success: function(data){
						...
						if(obj.isError=="true"){
							...
							if(isExist=="true"){
								...
								mainInfoJo.put("ddtype", 8);// 4 //类型，销售订单值为8，采购订单值为7
								...
								*/
								...
								for(var i=0;i<goodsListArrLen;i++){
									...
									objGoodsInfo["ddtype"] = 8;
									objGoodsInfo["cgddorganization"] = organization;
									objGoodsInfo["cgddsupplyunit"] = kehu;
									objGoodsInfo["cgddbrokerage"] = brokerage;
									objGoodsInfo["cgddreceivestorehouse"] = fahuocangku;
	...
	<script type="text/javascript">
		...
		//商品名称改变修改货号
		$("#cgddgoodsname").change(function(){
			if(cacheGoodsList!=null){
				var len = cacheGoodsList.length;
				for(var i=0;i<len;i++){
					var tmpGoods =  cacheGoodsList[i];
					if(tmpGoods.ptypeid == $("#cgddgoodsname").val()){
						showLoader("正在刷新商品颜色,尺寸和价格信息，请稍候...");
						$("#cgddgoodsnumber").val(tmpGoods.pusercode);
						//请求商品颜色，尺寸和价格
						sendForGoodsOtherInfo(function(){
							$("#cgddgoodscolor").selectmenu('refresh', true);
							$("#cgddgoodssize").selectmenu('refresh', true);
							$("#price").selectmenu('refresh', true);
							$("#sum").val("");
		...
		function sendForGoods(goodsCode,pageNum,backFunc){
			var organization =  $("#jigou").data("orgCode");
			var storeHouseID = $("#fahuocangku").val();
			var requestData = "action=action_xsdd_goods&organization="+organization+"&OperatorID=<%=OperatorID%>&storeHouseID="+storeHouseID+"&pageNum=1&itemsInEachPage=5";
			if(pageNum!=null){
				requestData = "action=action_xsdd_goods&organization="+organization+"&OperatorID=<%=OperatorID%>&storeHouseID="+storeHouseID+"&pageNum="+pageNum+"&itemsInEachPage=5&goodsCode="+goodsCode;
			}
			$.ajax({
				url: contextPath+"/BusinessServlet",
				data: requestData,
